...
func TestDeclarativeAPI(t *testing.T) {
	...
	ordersParam := props["orders"].(map[string]any)
	...
	resultParam := outputProps["result"].(map[string]any)
	...
	notificationsParam := outputProps["notifications"].(map[string]any)
	if notificationsParam["type"] != "array" {
		t.Errorf("Expected notifications type to be array, got %v", notificationsParam["type"])
	}
	notificationsItems := notificationsParam["items"].(map[string]any)
